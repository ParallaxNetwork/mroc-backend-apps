...
export const consentAdd = async (req, res) => {
    try {
        ...
        if (currConsent) {
            return sendReturn(400, "There is already active consent with current input", res);
        }
        await new Consent({
            _id: nanoid(),
            ownerId: req.user,
...
export const consentGetList = async (req, res) => {
    try {
        const consents = await Consent.find({ ownerId: req.user, isActive: true }, { _id: 1, ownerId: 1, receiverId: 1, fileId: 1 });
