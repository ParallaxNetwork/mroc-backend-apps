...
import { litIpfsEncrypt, litIpfsDecrypt, litConsentAuth, } from '../lib/litProtocol.js';
import { accessLocalWallet, genAuthSig } from '../lib/thirdWeb.js';
import File from '../models/File.js';
import { nanoid } from 'nanoid';
import * as dotenv from 'dotenv';
dotenv.config();
export const filePost = async (req, res) => {
    try {
...
export const fileGet = async (req, res) => {
    try {
        ...
        console.log(test);
        return res.status(200).send('OK');
        const decryptedFileBuffer = Buffer.from(await litIpfsDecrypt(currFile.cid, authSig));
